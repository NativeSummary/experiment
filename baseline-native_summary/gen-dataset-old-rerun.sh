...
for i in `ls $TARGET-results`; do
    ...
    # 没有重打包的apk文件
    if [ ! -e "$TARGET-results/$i/repacked_apks/$i" ]; then
        continue
    fi
    # 没有sinks文件
    if [ ! -e "$TARGET-results/$i/repacked_apks/$i.sinks.txt" ]; then
        >&2 echo error: $TARGET-results/$i/repacked_apks has no $i.sinks.txt
        exit -1
    fi
    # empty是正常的
    ...
    cp $TARGET-results/$i/repacked_apks/$i $OUT_PATH/$i
    cp $SS $OUT_PATH/$i.sources-sinks.txt
    cat $TARGET-results/$i/repacked_apks/$i.sinks.txt >> $OUT_PATH/$i.sources-sinks.txt
done
